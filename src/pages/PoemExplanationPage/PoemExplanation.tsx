import { FC, Fragment, Ref, useCallback, useMemo, useRef, useState } from "react";
...
const PoemExplanation: FC = (props) => {
	...
	const { error, loading, poem } = useGetPoem(id);
	...
	const getCurrentSection = useCallback((sections: Section[], secondsPassed: number) => {
			...
			(currentSection?.subsections as Subsection[]).map(
				(subsection, index) => {
					...
					const isLastIteration =
						index ===
						(currentSection?.subsections as Subsection[]).length -
							1;
					if (isFirstIteration && secondsPassed < subsection.time) {
						...
						secondsPassed >
							(currentSection.subsections as Subsection[])[
								index - 1
							].time &&
						secondsPassed < subsection.time
					) {
	...
	const verseClickHandler = (index: number) => {
		...
		const targetVideoSection = poem?.videoSections.find(
			(videoSection) =>
				videoSection.verseIndex === index &&
				videoSection.firstExplanation
		);
	...
	return (
		<PageComponent>
          ...
          <Helmet>
            ...
            <meta name="description" content={`شرح ویدئویی شعر ${poem.title}`}/>
            <meta name="keywords" content={keywordsText}/>
					...
					<div className="explanation-video-and-sections">
						<ExplanationSectionContainer
							...
							activeSection={
								(currentSection as SectionWithIndex)
									.sectionIndex
							}
					...
					<PoemScriptBox
						...
						activeVerseIndex={
							(currentSection as SectionWithIndex).verseIndex
						}
						verses={poem.verses}
						isPlaying={isPlaying}
					/>
