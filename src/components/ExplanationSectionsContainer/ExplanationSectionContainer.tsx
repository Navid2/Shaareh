...
const ExplanationSectionContainer: FC<ExplanationSectionContainerProps> = (
  ...
  useEffect(() => {
    document
      .querySelector(`.explanation-section-${props.activeSection}`)
      ?.scrollIntoView();
    ...
    setTimeout(() => {
      if (
        ...
        props.activeSection !== props.videoSections.length - 1
      ) {
        explanationSectionList?.scrollBy({
          left: 0,
          top: -30,
          behavior: "smooth",
        });
          ...
          return (
            <ExplanationVideoSection
              ...
              currentSubsectionIndex={
                isActive ? props.currentSubsectionIndex : -1
              }
              key={section.time}
            />
          );
